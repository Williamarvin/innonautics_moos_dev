...
void processImageData(const vector<char> &imageData)
{
  ...
  cv::Mat img =
      cv::imdecode(cv::Mat(imageData), cv::IMREAD_COLOR); // Decode image data
  ...
  // Define the path to save files: two directories up and inside a 'data'
  // folder
  const string dataDirectory =
      "../../../moos-ivp/ivp/data"; // Path to save files

  // Ensure that the data directory exists
  if (!filesystem::exists(dataDirectory))
  {
    filesystem::create_directories(
        dataDirectory); // Create directories if they do not exist
...
void getMap()
{
  ...
  string apiKey =
      "AIzaSyBvIZChwJJ0uZNUieXOP9092kOafpqXoeI"; // Replace with your actual API
                                                 // key
  string url =
      "https://maps.googleapis.com/maps/api/staticmap?center=" + lat_lon_input +
      "&zoom=14&size=800x800&key=" + apiKey; // Set size to 800x800
...
void PMV_MOOSApp::setPendingEventsPipe(
    Threadsafe_pipe<MOOS_event> *pending_moos_events)
{
...
void PMV_MOOSApp::handleAppCastRequesting(bool force)
{
  ...
  if (real_elapsed_time >= appcast_request_interval)
  {
    ...
    else if (refresh_mode == "events")
    {
      ...
      postAppCastRequest(current_node, current_proc, key_app, "any",
                         appcast_dur);
...
void PMV_MOOSApp::handleStartUp(const MOOS_event &e)
{
  ...
  for (p = sParams.rbegin(); p != sParams.rend(); p++)
  {
    ...
    if (param == "appcast_viewable")
      param = "infocast_viewable";
    if (param == "appcast_height")
      param = "infocast_height";
    if (param == "appcast_width")
      param = "infocast_width";
    if (param == "appcast_font_size")
      param = "infocast_font_size";
...
void PMV_MOOSApp::handlePendingPostsFromGUI()
{
  ...
  for (unsigned i = 0; i < cmd_posts.size(); i++)
  {
    ...
    if (valtype == "string")
    {
      ...
      if (strContains(sval, "$[targ]") && (cmd_targ != "local") &&
          (cmd_targ != "shore"))
...
void PMV_MOOSApp::postAppCastRequest(string channel_node, string channel_proc,
                                     string given_key, string threshold,
                                     double duration)
{
...
bool PMV_MOOSApp::buildReport()
{
  ...
  m_msgs << "------------------ " << endl;
  m_msgs << "Total GeoShapes:   " << m_gui->mviewer->shapeCount("total_shapes")
         << endl;
